                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card shadow">
                            <div class="card-body">
                                <form action="{{ route('backend.admin.periode.update', ['id' => $item->id]) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                        ...
                                        <div class="col-sm-10">
                                            <select name="bulan" class="form-control" id="bulan" required>
                                                ...
                                                @foreach ($bulan as $key => $value)
                                                    <option value="{{ $value }}" {{ ($value == $item->bulan ? "selected" : "") }}>{{ $value }}</option>
                                                @endforeach
                                            </select>
                                            @if ($errors->has('bulan'))
