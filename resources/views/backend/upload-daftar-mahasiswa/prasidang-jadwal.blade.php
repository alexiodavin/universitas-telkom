                        ...
                        <div class="card shadow">
                            <div class="card-body">
                                @if(session('auth_login') == 'koordinator_pa')
                                    <form action="{{ route('backend.koordinator-pa.upload-daftar-mahasiswa.prasidang.input-jadwal-prasidang.update', ['id' => $item->id]) }}" method="POST" enctype="multipart/form-data">
                                @else
                                    <form action="{{ route('backend.admin.upload-daftar-mahasiswa.prasidang.input-jadwal-prasidang.update', ['id' => $item->id, 'jadwal_id' => $jadwal_prasidang->id]) }}" method="POST" enctype="multipart/form-data">
                                @endif
                                @csrf
                                    @if(session('auth_login') == 'koordinator_pa')
                                        <div class="form-group row">
                                        ...
                                        <label class="col-sm-2 col-form-label font-weight-bold">Tanggal</label>
                                            <div class="col-sm-10">
                                                <input type="date" class="form-control" name="tanggal_prasidang" value="{{ $jadwal_prasidang->tanggal_prasidang ?? null }}" required>
                                        ...
                                        <label class="col-sm-2 col-form-label font-weight-bold">Jam</label>
                                            <div class="col-sm-10">
                                                <div class="row p-0 m-0">
                                                    <input style="width: 200px;" type="time" class="form-control mr-3" name="jam_mulai_prasidang" value="{{ $jadwal_prasidang->jam_mulai_prasidang ?? null }}" required>
                                                    <input style="width: 200px;" type="time" class="form-control" name="jam_selesai_prasidang" value="{{ $jadwal_prasidang->jam_selesai_prasidang ?? null }}" required>
                                    ...
                                    @else
                                        ...
                                        <label class="col-sm-2 col-form-label font-weight-bold">Ruangan<span style="color:red;"> *</span></label>
                                            ...
                                            <select name="ruangan_id" class="form-control" required>
                                                ...
                                                <option value="" @if(str_contains($jadwal_prasidang->ruangan ?? '', 'http')) selected @endif>Online</option>
                                                @foreach ($ruangans as $ruangan)
                                                    <option value="{{ $ruangan->id }}" @if($jadwal_prasidang->ruangan_id ?? 0 == $ruangan->id) selected @endif>{{ $ruangan->nama }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <label class="col-sm-2 col-form-label font-weight-bold">Link Zoom / Google Meet</label>
                                            <div class="col-sm-10">
                                                @if($jadwal_prasidang)
                                                    <input type="text" class="form-control" name="ruangan" value="{{ $jadwal_prasidang->ruangan_id == null ? $jadwal_prasidang->ruangan : '' }}">
                                                @else
