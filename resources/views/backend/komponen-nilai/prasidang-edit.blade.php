...
@section('content')
    <div class="content-wrapper">
        ...
        <form action="{{ route('backend.koordinator-pa.komponen-nilai.prasidang.update') }}" method="POST" enctype="multipart/form-data">
            @csrf
                                        ...
                                        <tbody>
                                            @foreach ($komponen_prasidangs as $kn)
                                                <tr>
                                                    ...
                                                    <td style="border-right: 2px solid #dee2e6; text-align: center;">
                                                        <a href="{{ route('backend.koordinator-pa.komponen-nilai.prasidang.delete', ['id' => $kn->id]) }}" class="btn btn-primary shadow bg-primary" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini ?')"> <i class="fa fa-trash"></i> Delete</a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                            <tr>
...
@section('js')
    <script>
        $('select').on('change', function() {
            return window.location.href = "{{ url('koordinator-pa/komponen-nilai/prasidang/edit') }}/"+this.value
        });
