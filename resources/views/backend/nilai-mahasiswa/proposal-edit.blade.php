...
@section('content')
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card shadow">
                            <div class="card-body">
                                <form
                                    action="{{ route('backend.dosen.nilai-mahasiswa.proposal.update-nilai', ['id' => $item->id]) }}"
                                                            ...
                                                            @if ($item->nilai_penguji1)
                                                                @foreach ($item->nilai_penguji1->detail_nilai as $nilai)
                                                                    @if ($kn->id == $nilai->komponen_proposal_id)
                                                                        <input type="number"
                                                                            name="nilai[{{ $nilai->nilai }}]"
                                                                            class="form-control"
                                                                            value="{{ $nilai->nilai }}" readonly>
                                                                    @endif
                                                                @endforeach
                                                            @else
                                    ...
                                    @if ($item->penguji1_id == auth()->user()->dosen->id)
                                        ...
                                        @endif
                                        @if ($item->nilai_penguji1)
                                            {{-- <button class="btn btn-primary float-right" id="1"
                                                onclick="editNilai(this.id)" type="button" style="margin: 0 25px 0 0">Edit
                                                Nilai</button> --}}
                                            <a href="{{ route('backend.dosen.nilai-mahasiswa.proposal.edit-detail', ['id' => $item->id]) }}"
                                                class="btn btn-primary float-right" style="margin: 0 25px 0 0">Edit
                                                Nilai</a>
                                        @endif
                                    @elseif ($item->penguji2_id == auth()->user()->dosen->id)
                                        ...
                                        @endif
                                        @if ($item->nilai_penguji2)
                                            {{-- <button class="btn btn-primary float-right" id="2"
                                                onclick="editNilai(this.id)" type="button" style="margin: 0 25px 0 0">Edit
                                                Nilai</button> --}}
                                            <a href="{{ route('backend.dosen.nilai-mahasiswa.proposal.edit-detail', ['id' => $item->id]) }}"
                                                class="btn btn-primary float-right" style="margin: 0 25px 0 0">Edit
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="card shadow">
                            <div class="card-body">
                                <h4>Catatan Revisi Proposal</h4>
                                <table class="table table-hover border" style="width: 100%; border: 0; ">
                                    <tbody>
                                        <tr>
                                            <form
                                                action="{{ route('backend.dosen.nilai-mahasiswa.proposal.update', ['id' => $item->id]) }}"
                                                method="POST" enctype="multipart/form-data">
                                                @csrf
                                                <td style="border-right: 2px solid #dee2e6; " class="col-8">
                                                    <div class="form-group row">
                                                        <label class="col-sm-2 col-form-label font-weight-bold">Penguji 1
                                                            <span style="color: red">*</span></label>
                                                        <div class="col-sm-10">
                                                            <div class="d-flex flex-row">
                                                                <input type="file" name="file"
                                                                    accept=".pdf,.doc,.docx" class="w-50 form-control"
                                                                    @if ($item->penguji1_id != auth()->user()->dosen->id) disabled @endif>
                                                                <div class="font-weight-bold pt-1 mx-3">Masukkan File
                                                                    Kontrak proposal</div>
                                                            </div>
                                                    ...
                                                    @if ($item->nilai_penguji1->file ?? null)
                                                        @if ($item->penguji1_id == auth()->user()->dosen->id)
                                                            <a target="_blank"
                                                                href="{{ asset('file/' . $item->nilai_penguji1->file) }}"
                                                    ...
                                                    <div class="form-group row">
                                                        <label for=""
                                                            class="col-sm-2 col-form-label font-weight-bold">Catatan <span
                                                                style="color: red">*</span></label>
                                                        <textarea name="catatan" class="form-control" style="margin: 0 7.5px; width:94%"
                                                            @if ($item->penguji1_id != auth()->user()->dosen->id) disabled @endif>{{ $item->nilai_penguji1->catatan ?? '' }}</textarea>
                                                    </div>
                                                </td>
                                                        ...
                                                        @if ($item->nilai_penguji1)
                                                            ...
                                                            @else
                                                                <button type="submit" class="btn btn-primary"
                                                                    @if ($item->penguji1_id != auth()->user()->dosen->id) disabled @endif>Approve</button>
                                                            @endif
                                                        @else
                                                            <button type="submit" class="btn btn-primary"
                                                                @if ($item->penguji1_id != auth()->user()->dosen->id) disabled @endif>Approve</button>
                                        ...
                                        @if ($item->jumlah_penguji == 2)
                                            <tr>
                                                <form
                                                    ...
                                                    <td style="border-right: 2px solid #dee2e6;" class="col-8">
                                                        <div class="form-group row">
                                                            <label class="col-sm-2 col-form-label font-weight-bold">Penguji
                                                                2 <span style="color: red">*</span></label>
                                                            <div class="col-sm-10">
                                                                <div class="d-flex flex-row">
                                                                    <input type="file" name="file"
                                                                        accept=".pdf,.doc,.docx" class="w-50 form-control"
                                                                        @if ($item->penguji2_id != auth()->user()->dosen->id) disabled @endif>
                                                                    <div class="font-weight-bold pt-1 mx-3">Masukkan File
                                                                        Kontrak proposal</div>
                                                                </div>
                                                                <i>Format file .docx, .pdf, .doc</i>
                                                        ...
                                                        @if ($item->nilai_penguji2->file ?? null)
                                                            @if ($item->penguji2_id == auth()->user()->dosen->id)
                                                                <a target="_blank"
                                                                    href="{{ asset('file/' . $item->nilai_penguji2->file) }}"
                                                                    class="btn btn-primary">Lihat File</a>
                                                                <br><br>
                                                            @endif
                                                        @endif
                                                        <div class="form-group row">
                                                            <label for=""
                                                                class="col-sm-2 col-form-label font-weight-bold">Catatan
                                                                <span style="color: red">*</span></label>
                                                            <textarea name="catatan" class="form-control" style="margin: 0 7.5px; width:94%"
                                                                @if ($item->penguji2_id != auth()->user()->dosen->id) disabled @endif>{{ $item->nilai_penguji2->catatan ?? '' }}</textarea>
                                                        </div>
                                                    </td>
                                                    <td style="border-right: 2px solid #dee2e6; text-align: center;">
                                                        <center>
                                                            ...
                                                            @if ($item->nilai_penguji2)
                                                                ...
                                                                @else
                                                                    <button type="submit" class="btn btn-primary"
                                                                        @if ($item->penguji2_id != auth()->user()->dosen->id) disabled @endif>Approve</button>
                                                                @endif
                                                            @else
                                                                <button type="submit" class="btn btn-primary"
                                                                    @if ($item->penguji2_id != auth()->user()->dosen->id) disabled @endif>Approve</button>
        ...
        <center>
            @if ($item->nilai_final == null)
                @if ($item->nilai_penguji1 && $item->nilai_penguji2)
                    <a href="{{ route('backend.dosen.nilai-mahasiswa.proposal.set-nilai-akhir', ['id' => $item->id]) }}"
                        class="btn btn-primary">Simpan Nilai Akhir</a>
                    <br><br>
                @endif
            @endif
        </center>
    ...
    <script>
        ...
        const editNilai = (e) => {
            ...
            // console.log(id);
            $.ajax({
                url: '/dosen/nilai-mahasiswa/proposal/{id}/update-nilai',
                type: 'POST',
                data: {
                    'id': id,
                    'penguji': e,
                ...
                success: function(response) {
                    ...
                    window.location.href = `/dosen/nilai-mahasiswa/proposal/${response.id}/edit`
                },
                error: function(err) {
